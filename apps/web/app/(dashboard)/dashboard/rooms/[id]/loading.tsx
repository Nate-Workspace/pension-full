      ...
      <section className="grid gap-4 xl:grid-cols-[1.15fr_1fr]">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-1.5">
            {Array.from({ length: 7 }, (_, index) => (
              <div key={`calendar-header-${index}`} className="h-7 animate-pulse rounded-md bg-slate-100" />
            ))}
            ...
            <table className="w-full min-w-160 border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  {Array.from({ length: 5 }, (_, index) => (
                    <th key={`room-booking-head-${index}`} className="px-4 py-3">
                      <div
                        className={`h-3 animate-pulse rounded bg-slate-100 ${
                          index === 0 ? "w-24" : index === 1 ? "w-16" : index === 2 ? "w-12" : index === 3 ? "w-20" : "w-12"
                        }`}
                      />
                    </th>
                  ))}
              ...
              <tbody>
                {Array.from({ length: 5 }, (_, rowIndex) => (
                  <tr key={`room-booking-row-${rowIndex}`} className="border-b border-slate-100 last:border-0">
                      ...
                      >
                        <div
                          className={`h-4 animate-pulse rounded bg-slate-100 ${
                            cellIndex === 0 ? "w-36" : cellIndex === 1 ? "w-28" : cellIndex === 2 ? "w-20" : "ml-auto w-16"
                          }`}
                        />
                      </td>
                    ))}
